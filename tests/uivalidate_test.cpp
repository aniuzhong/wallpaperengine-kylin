...
private slots:
    ...
    void sessionCookieIsFoundAmongOthers () {
        QCOMPARE (Ui::sessionTokenFromCookie ("other=1; lwe_session=deadbeef; theme=dark", "lwe_session"),
                  std::string ("deadbeef"));
        QCOMPARE (Ui::sessionTokenFromCookie ("lwe_session=only", "lwe_session"), std::string ("only"));
        QCOMPARE (Ui::sessionTokenFromCookie ("lwe_session=trailing;", "lwe_session"), std::string ("trailing"));
    }

    void sessionCookieRejectsNearMisses () {
        QCOMPARE (Ui::sessionTokenFromCookie ("", "lwe_session"), std::string ());
        QCOMPARE (Ui::sessionTokenFromCookie ("other=1", "lwe_session"), std::string ());
        // a cookie whose name merely ends with ours is not ours
        QCOMPARE (Ui::sessionTokenFromCookie ("not_lwe_session=nope", "lwe_session"), std::string ());
    }
