...
private slots:
    void cmdlineMatchesTheDesktopProcess () {
        QVERIFY (Integration::isPeonyDesktopCmdline (cmdline ({ "/usr/bin/peony-qt-desktop", "-w", "-d" })));
        QVERIFY (Integration::isPeonyDesktopCmdline (cmdline ({ "peony-qt-desktop" })));
    }

    void cmdlineRejectsEverythingElse () {
        QVERIFY (!Integration::isPeonyDesktopCmdline (cmdline ({ "/usr/bin/peony", "--daemon" })));
        QVERIFY (!Integration::isPeonyDesktopCmdline (""));
    }
    ...
    void wallpaperListOrdersPreviousNormalizedMarker () {
        ...
        QCOMPARE (Integration::buildWallpaperList ("/data/marker.png", "/var/lib/AccountsService/backgrounds/x.png",
                                                   "/old/wall.png"),
    ...
    void wallpaperListDropsEmptyAndRepeatedEntries () {
        QCOMPARE (Integration::buildWallpaperList ("/m.png", "", ""), std::string ("/m.png"));
        // accountsservice handed back the marker itself: one entry, not two
        QCOMPARE (Integration::buildWallpaperList ("/m.png", "/m.png", ""), std::string ("/m.png"));
        // the accountsservice write failed: the previous wallpaper is the
        // only other candidate the shim may be asked about
        QCOMPARE (Integration::buildWallpaperList ("/m.png", "", "/old.png"), std::string ("/old.png:/m.png"));
    }
    ...
    void wallpaperListKeepsPathsThatMerelyContainEachOther () {
        ...
        QCOMPARE (Integration::buildWallpaperList ("/a/b.png.bak", "", "/a/b.png"),
    ...
    void shimEnvironmentCarriesTheThreeVariables () {
        ...
        QCOMPARE (env.at ("LD_PRELOAD"), std::string ("/opt/lwe/lib/libpeony-alpha-shim.so"));
