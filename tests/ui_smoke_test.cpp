...
// "Set-Cookie: wallpaper_engine_session=<token>" out of a response, as the "name=value"
// pair a client would send back.
std::string sessionCookieOf(const std::string& response) {
    const size_t at = response.find("wallpaper_engine_session=");
...
int main() {
    char tempTemplate[] = "/tmp/wallpaper-engine-ui-smoke-XXXXXX";
    ...
    check(contains(bodyOf(status), "\"unit\":\"wallpaper-engine-ui-smoke-test\""), "status names the unit under test");
