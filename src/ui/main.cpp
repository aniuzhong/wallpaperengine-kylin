// lwe-control: UI and CLI control surface for the Kylin linux-wallpaperengine
...
int main (int argc, char** argv) {
    ...
    QCoreApplication::setApplicationName ("lwe-control");

    const QStringList args = QCoreApplication::arguments ();

    if (args.contains ("--selftest")) {
	// load the config (creating defaults on first run) and report
	const auto config = LWE::Config::load ();
	std::printf ("config path: %s\n", LWE::Config::configPath ().toUtf8 ().constData ());
    ...
    std::printf ("lwe-control: UI scaffold — nothing to show yet\n");
    std::printf ("usage: lwe-control [--selftest]\n");
    return 0;
}
